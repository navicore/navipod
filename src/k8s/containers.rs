...
fn parse_log_line(line: &str) -> LogRec {
    ...
    let regex = TIMESTAMP_REGEX.get_or_init(|| {
        ...
        Regex::new(
            r"^([0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}(?:\.[0-9]+)?Z?)\s+(.*)$",
        )
            ...
            probe.tcp_socket.as_ref().map_or_else(
                || {
                    probe.exec.as_ref().map_or_else(
                        || ("Unknown".to_string(), "No handler specified".to_string()),
                        |exec| {
                            let command = exec.command.as_ref().map_or_else(
                                || "No command specified".to_string(),
                                |cmd| cmd.join(" "),
                            );
                            ("Exec".to_string(), format!("Run: {command}"))
                        },
                    )
                },
                |tcp_socket| {
                    let port = match &tcp_socket.port {
                        ...
                        k8s_openapi::apimachinery::pkg::util::intstr::IntOrString::String(port) => {
                            port.clone()
                        }
                    };
                    let host = tcp_socket.host.as_deref().unwrap_or("localhost");
                    ("TCP".to_string(), format!("Connect to {host}:{port}"))
                },
            )
        },
        |http_get| {
...
fn extract_container_probes(
    ...
    if let Some(liveness_probe) = &container.liveness_probe {
        ...
        debug!(
            "Found liveness probe: {} - {}",
            probe.handler_type, probe.details
        );
        probes.push(probe);
    }
    ...
    if probes.is_empty() {
        ...
        debug!(
            "Extracted {} total probes/endpoints for container: {}",
            probes.len(),
            container.name
        );
    }
...
fn extract_container_resources(
    container: &k8s_openapi::api::core::v1::Container,
) -> (
    Option<String>,
    Option<String>,
    ...
    let (cpu_request, memory_request) = container
        .resources
        .as_ref()
        ...
        .map_or((None, None), |requests| {
            let cpu_req = requests
                .get("cpu")
                .and_then(|q| parse_cpu(&q.0).map(format_cpu));
            let mem_req = requests
                .get("memory")
                .and_then(|q| parse_memory(&q.0).map(format_memory));
            (cpu_req, mem_req)
        });

    let (cpu_limit, memory_limit) = container
        .resources
        .as_ref()
...
fn extract_metrics_from_annotations(
    annotations: &std::collections::BTreeMap<String, String>,
    _container_ports: Option<&Vec<k8s_openapi::api::core::v1::ContainerPort>>,
) -> Vec<ContainerProbe> {
    ...
    if annotations.get("prometheus.io/scrape").map(String::as_str) == Some("true") {
        let metrics_path = annotations
            .get("prometheus.io/path")
            .cloned()
            .unwrap_or_else(|| "/metrics".to_string());
        let metrics_port = annotations
    ...
    if annotations
        .get("navipod.io/metrics-enabled")
        .map(String::as_str)
        == Some("true")
    {
        let metrics_path = annotations
            .get("navipod.io/metrics-path")
            .cloned()
            .unwrap_or_else(|| "/metrics".to_string());
        let metrics_port = annotations
            .get("navipod.io/metrics-port")
            .and_then(|p| p.parse::<i32>().ok())
            .unwrap_or(8080);

        // Only add if not already added by prometheus annotations
        if !metrics_probes.iter().any(|p| {
            p.details
                .contains(&format!(":{metrics_port}{metrics_path}"))
        }) {
            metrics_probes.push(ContainerProbe {
                probe_type: "Metrics".to_string(),
                handler_type: "HTTP".to_string(),
    ...
    debug!(
        "Extracted {} metrics endpoints from annotations",
        metrics_probes.len()
    );
    metrics_probes
}
...
pub async fn list(selector: BTreeMap<String, String>, pod_name: String) -> Result<Vec<Container>> {
    use crate::k8s::metrics_client::{create_metrics_lookup, fetch_pod_metrics};
    use tracing::debug;
...
fn process_pod_list(
    ...
    pod_name: String,
    metrics_lookup: std::collections::HashMap<
        String,
        std::collections::HashMap<String, crate::k8s::metrics_client::ContainerMetric>,
    >,
...
pub async fn logs_enhanced(
    ...
    let logs_string = match pods.logs(&pod_name, &log_params).await {
        ...
        Err(e) => {
            warn!(
                "Failed to get logs for pod {}, container {}: {}",
    ...
    debug!(
        "Retrieved logs for pod: {}, container: {}, follow: {}",
    ...
    debug!(
        "Parsed {} log lines for pod: {}, container: {}",
        log_vec.len(),
        pod_name,
        container_name
    );
