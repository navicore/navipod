...
impl ProbeExecutor {
    ...
    pub async fn execute_probe(&self, probe: &ContainerProbe) -> ProbeResult {
        ...
        debug!(
            "Executing {} probe for pod {}: {}",
            probe.probe_type, self.pod_name, probe.details
        );
        ...
        let result = match probe.handler_type.as_str() {
            ...
            _ => ProbeResult {
                ...
                error_message: Some(format!(
                    "Unsupported probe handler type: {}",
                    probe.handler_type
                )),
        ...
        debug!(
            "Probe {} completed in {}ms with status {:?}",
    ...
    async fn execute_tcp_probe(&self, probe: &ContainerProbe) -> ProbeResult {
        ...
        let port = self
            .extract_port_from_details(&probe.details)
            .unwrap_or(8080);
    ...
    async fn execute_exec_probe(&self, probe: &ContainerProbe) -> ProbeResult {
        ProbeResult {
            ...
            response_body: format!(
                "Exec probe not supported via port-forward: {}",
                probe.details
            ),
    ...
    async fn ensure_port_forward(&self, remote_port: u16) -> Result<u16> {
        ...
        debug!(
            "Starting port-forward for pod {} port {} -> {}",
            self.pod_name, remote_port, local_port
        );
        ...
        match cmd.spawn() {
            Ok(mut child) => {
                ...
                if self.test_local_port(local_port).await {
                    debug!(
                        "Port-forward established successfully for port {}",
                        local_port
                    );
                    Ok(local_port)
                } else {
                    ...
                    Err(crate::error::Error::Custom(format!(
                        "Port-forward failed to establish for port {local_port}"
                    )))
                }
            }
            Err(e) => {
    ...
    fn parse_http_details(&self, details: &str) -> (String, String) {
        ...
        if details.starts_with("GET ")
            || details.starts_with("POST ")
            || details.starts_with("HEAD ")
        {
