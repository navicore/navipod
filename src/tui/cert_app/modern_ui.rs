...
fn render_header(f: &mut Frame, app: &App, area: Rect, theme: &NaviTheme) {
    ...
    let valid_count = certs
        .iter()
        .filter(|c| c.is_valid() == "true" || c.is_valid().to_lowercase() == "valid")
        .count();
    let expired_count = certs
        .iter()
        .filter(|c| is_expired_soon(c.expires()))
        .count();
...
fn render_cert_list(f: &mut Frame, app: &App, area: Rect, theme: &NaviTheme) {
    ...
    let scroll_offset =
        if UiHelpers::safe_cast_u16(selected_index, "cert scroll offset") >= visible_cards {
...
fn render_cert_card(
    ...
    let content = vec![
        ...
        Line::from(vec![
            Span::raw("    Issuer: "),
            Span::styled(
                truncate_text(cert.issued_by(), 40),
                theme.text_style(TextType::Body),
            ),
        ]),
        Line::from(vec![
...
fn render_filter_modal(f: &mut Frame, app: &App, theme: &NaviTheme) {
    ...
    let cursor_pos = Position {
        x: modal_area.x
            + UiHelpers::safe_cast_u16(app.get_cursor_pos(), "cert cursor position")
            + 1,
        y: modal_area.y + 1,
    };
