...
fn render_header(f: &mut Frame, app: &App, area: Rect, theme: &NaviTheme) {
    ...
    let restart_count = containers
        .iter()
        .filter(|c| UiHelpers::safe_parse_i32(c.restarts(), "container_restarts") > 0)
        .count();
...
fn render_container_card(
    f: &mut Frame,
    container: &crate::tui::data::Container,
    area: Rect,
    is_selected: bool,
    theme: &NaviTheme,
) {
    ...
    let (restart_bar, restart_color) = if restart_count > 0 {
        let health_ratio = if restart_count <= 2 {
            0.75
        ...
        UiHelpers::health_progress_bar(
            ...
            // Intentional for progress bar display
            if health_ratio > 0.0 {
                (health_ratio * 8.0) as usize
            } else {
                0
            },
            8,
    ...
    let cpu_display =
        format_resource_display(container.cpu_usage.as_ref(), container.cpu_limit.as_ref());
    let cpu_percentage =
        calculate_resource_percentage(container.cpu_usage.as_ref(), container.cpu_limit.as_ref());
    ...
    let memory_display = format_resource_display(
        container.memory_usage.as_ref(),
        container.memory_limit.as_ref(),
    );
    let memory_percentage = calculate_resource_percentage(
        container.memory_usage.as_ref(),
        container.memory_limit.as_ref(),
    );
    ...
    let (cpu_sparkline, cpu_trend) = get_metrics_history()
        ...
        .and_then(|history| {
            history
                .get_container_history(&container.pod_name, &container.name)
                .map(|ts| {
    ...
    let (memory_sparkline, memory_trend) = get_metrics_history()
        ...
        .and_then(|history| {
            history
                .get_container_history(&container.pod_name, &container.name)
                .map(|ts| {
                    let limit_bytes = container
                        .memory_limit
                        .as_ref()
                        .and_then(|l| parse_memory(l));
    ...
    let mut content = vec![
        ...
        Line::from(vec![
            ...
            Span::raw("  Restarts: "),
            Span::styled(
                format!("{restart_count} "),
                get_restart_style(restart_count, theme),
            ),
    ...
    content.push(Line::from(vec![
        Span::raw("    "),
        Span::styled(
            truncate_text(container.description(), 60),
            theme.text_style(TextType::Caption),
        ),
    ]));
...
fn render_mounts_section(f: &mut Frame, app: &mut App, area: Rect, theme: &NaviTheme) {
    ...
    let mounts_list = List::new(mount_items).block(
        Block::default()
            .title(title)
            .borders(Borders::ALL)
...
fn render_env_vars_section(f: &mut Frame, app: &mut App, area: Rect, theme: &NaviTheme) {
    ...
    let title = if is_focused {
        format!(
            "{} Environment Variables [TAB: next panel]",
            Symbols::SETTINGS
        )
    } else {
    ...
    let env_list = List::new(env_items).block(
        Block::default()
            .title(title)
            .borders(Borders::ALL)
            .border_style(border_style)
...
fn render_probes_section(f: &mut Frame, app: &mut App, area: Rect, theme: &NaviTheme) {
    ...
    let probes_widget = List::new(probe_items).block(
        Block::default()
            .title(title)
            .borders(Borders::ALL)
            .border_style(border_style)
    ...
    if is_focused && !probes.is_empty() {
        ...
        let mut scrollbar_state =
            ratatui::widgets::ScrollbarState::new(probes.len().saturating_sub(1))
...
fn render_probe_results_popup(f: &mut Frame, app: &App, theme: &NaviTheme) {
    ...
    if let Some(ref result) = app.current_probe_result {
        ...
        render_probe_popup_scrollbar(
            f,
            modal_area,
            total_lines,
            content_height,
            app.probe_popup_scroll,
        );
        render_probe_popup_help(f, modal_area, theme);
    }
}

fn build_probe_result_lines(
    result: &crate::k8s::probes::ProbeResult,
    theme: &NaviTheme,
) -> Vec<Line<'static>> {
    ...
    all_lines.push(Line::from(vec![Span::styled(
        format!(
            "{} {} {} Probe Result",
            status_symbol, result.probe_type, result.handler_type
        ),
        Style::default()
            .fg(status_color)
            .add_modifier(Modifier::BOLD),
...
const fn get_probe_status_display(
    status: &crate::k8s::probes::ProbeStatus,
    theme: &NaviTheme,
) -> (&'static str, Color) {
    match status {
...
fn render_probe_popup_scrollbar(
    f: &mut Frame,
    modal_area: Rect,
    total_lines: usize,
    content_height: u16,
    ...
    if total_lines > content_height as usize {
        ...
        let mut scrollbar_state =
            ratatui::widgets::ScrollbarState::new(total_lines).position(scroll_position);
...
fn calculate_resource_percentage(
    usage_str: Option<&String>,
...
fn format_resource_display(usage: Option<&String>, limit: Option<&String>) -> String {
    match (usage, limit) {
        (Some(u), Some(l)) => calculate_resource_percentage(Some(u), Some(l))
            .map_or_else(|| format!("{u}/{l}"), |pct| format!("{u}/{l} [{pct:.0}%]")),
