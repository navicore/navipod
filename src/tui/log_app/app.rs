...
impl AppBehavior for log_app::app::App {
    ...
    fn stream(&self, should_stop: Arc<AtomicBool>) -> impl Stream<Item = Message> {
        ...
        tokio::spawn(async move {
            debug!(
                "Starting enhanced log stream for pod: {}, container: {}",
            ...
            .await
            {
                Ok(initial_logs) => {
                    if !initial_logs.is_empty()
                        && tx.send(Message::Log(initial_logs)).await.is_err()
                    {
                    ...
                    Ok(all_logs) => {
                        ...
                        if current_log_count > last_log_count {
                            let new_logs: Vec<LogRec> = if last_log_count == 0 {
                                ...
                                all_logs
                                    .into_iter()
                                    .skip(current_log_count.saturating_sub(50))
                                    .collect()
                            } else {
            ...
            debug!(
                "Log stream ended for pod: {}, container: {}",
                pod_name, container_name
            );
        });
...
impl App {
    ...
    fn handle_detail_popup_keys(&mut self, key: &crossterm::event::KeyEvent) -> Apps {
        use KeyCode::{Char, Down, Esc, PageDown, PageUp, Up};
    ...
    pub fn handle_navigation_key(&mut self, key: &crossterm::event::KeyEvent) -> bool {
        use KeyCode::{Char, Down, End, Home, PageDown, PageUp, Up};
            ...
            Message::Key(Event::Key(key)) => {
                if key.kind == KeyEventKind::Press {
                    ...
                    return match handle_common_keys(self, key, |app| Apps::Log { app }) {
                        ...
                        | KeyHandlerResult::Handled(app_holder) => app_holder,
