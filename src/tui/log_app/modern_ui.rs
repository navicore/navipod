...
fn render_header(f: &mut Frame, app: &App, area: Rect, theme: &NaviTheme) {
    ...
    let error_count = logs
        .iter()
        .filter(|l| {
            l.level.to_lowercase().contains("error") || l.level.to_lowercase().contains("err")
        })
        .count();
    let warn_count = logs
        .iter()
        .filter(|l| l.level.to_lowercase().contains("warn"))
        .count();
...
fn render_log_entry(
    ...
    let available_width = area
        .width
        .saturating_sub(spans.iter().map(|s| s.content.len() as u16).sum::<u16>() + 1)
        as usize;
    ...
    let log_entry = Paragraph::new(content).style(Style::default().bg(log_bg));
...
fn render_footer(f: &mut Frame, app: &App, area: Rect, theme: &NaviTheme) {
    let base_text =
        "↑↓: Navigate • /: Filter • t: Toggle Tailing • G: Jump to Latest • g: Jump to Top";
...
fn render_filter_modal(f: &mut Frame, app: &App, theme: &NaviTheme) {
    ...
    let help_text = vec![
        Line::from(
            "ESC: Cancel • Enter: Apply • Examples: '(error|warn)', 'failed.*request', '^INFO'",
        ),
...
fn get_log_level_style(level: &str, theme: &NaviTheme) -> Style {
    match level.to_lowercase().as_str() {
        s if s.contains("error") || s.contains("err") || s.contains("fatal") => {
...
fn render_log_detail_popup(f: &mut Frame, app: &App, theme: &NaviTheme) {
    ...
    if let Some(ref log_entry) = app.selected_log_detail {
        ...
        let text_height =
            calculate_wrapped_height(&log_entry.message, modal_area.width.saturating_sub(2)) + 6; // +6 for header lines
        if text_height > content_height as usize {
            ...
            let mut scrollbar_state = ratatui::widgets::ScrollbarState::new(
                text_height.saturating_sub(content_height as usize),
            )
            .position(
                app.detail_popup_scroll
                    .min(text_height.saturating_sub(content_height as usize)),
            );
