...
fn render_header(f: &mut Frame, app: &App, area: Rect, theme: &NaviTheme) {
    ...
    // Actions/shortcuts
    let actions_text = "f: filter • y: yaml • Enter: containers • c: colors • q: quit";
...
fn render_pod_card(
    ...
    let cpu_percentage =
        calculate_resource_percentage(pod.cpu_usage.as_ref(), pod.cpu_limit.as_ref());
    ...
    let memory_display =
        format_resource_display(pod.memory_usage.as_ref(), pod.memory_limit.as_ref());
    let memory_percentage =
        calculate_resource_percentage(pod.memory_usage.as_ref(), pod.memory_limit.as_ref());
    ...
    let node_display = if let (Some(node_name), Some(cpu_pct), Some(mem_pct)) = (
        &pod.node_name,
        pod.node_cpu_percent,
        pod.node_memory_percent,
    ) {
    ...
    content.push(Line::from(vec![
        Span::raw("    "),
        Span::styled(
            truncate_text(&pod.description, 60),
...
fn render_footer(f: &mut Frame, area: Rect, theme: &NaviTheme) {
    let footer_text = "Enter: Containers • e: Events • ↑↓: Navigate • G: Bottom • g: Top • Ctrl+F/B: Page Up/Down";
