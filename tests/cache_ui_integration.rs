...
use navipod::{
    cache_manager,
    k8s::cache::{DataRequest, FetchResult},
};
...
async fn test_cache_manager_initialization() {
    ...
    cache_manager::initialize_cache("default".to_string())
        .await
        .unwrap();
    ...
    let _result = tokio::time::timeout(std::time::Duration::from_millis(100), rx.recv()).await;
