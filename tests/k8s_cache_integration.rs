...
async fn test_cache_with_real_k8s() {
    ...
    // Store in cache
    cache.put(&request, FetchResult::ReplicaSets(real_data.clone()))
        .await
...
async fn test_background_fetcher_with_k8s() {
    ...
    fetcher.schedule_fetch(request.clone(), FetchPriority::High).await;
    ...
    let cached = cache.get(&request).await;
    assert!(cached.is_some(), "Background fetcher should have populated cache");
...
async fn test_cache_invalidation_and_refresh() {
    ...
    cache.put(&request, FetchResult::ReplicaSets(data))
        .await
        .expect("Failed to cache data");
    ...
    for i in 0..5 {
        ...
        let handle = tokio::spawn(async move {
            ...
            if let Ok(data) = list_replicas().await {
                ...
                cache_clone.put(&request, result).await.expect("Cache put failed");
